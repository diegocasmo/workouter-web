...
describe('<Select/>', () => {
  ...
  it('renders', () => {
    ...
    props.options.forEach((opt, idx) => {
      expect(wrapper.find(Field).props().children[idx].props.value).to.be.equal(
        props.options[idx].value
      )
      expect(
        wrapper.find(Field).props().children[idx].props.children
  ...
  it('renders validation errors', async () => {
    ...
    wrapper
      .find('select')
      .simulate('change', { target: { id: 'number', value: -3 } })
