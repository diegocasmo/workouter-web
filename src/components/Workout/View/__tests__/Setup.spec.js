...
describe('<WorkoutSetup/>', () => {
  ...
  it('renders', () => {
    ...
    expect(wrapper.find('.wkr-workout-setup__workout-name').text()).to.be.equal(`Name: ${props.name}`)
    expect(wrapper.find('.wkr-workout-setup__workout-rounds').text()).to.be.equal(`Rounds: ${props.rounds}`)
    expect(wrapper.find('.wkr-workout-setup__workout-rest-time-per-round').text())
      .to.be.equal(`Rest time per round: ${props.restTimePerRound} ${UNITS.SECONDS.text}`)
    expect(wrapper.find('.wkr-workout-setup__workout-rest-time-per-exercise').text())
