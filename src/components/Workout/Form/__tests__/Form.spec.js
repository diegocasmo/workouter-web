...
describe('<WorkoutForm/>', () => {
  ...
  it('renders <Prompt/>', () => {
    ...
    expect(wrapper.find(Prompt).props().message).to.be.equal(
  ...
  it('can create a valid workout', async () => {
    ...
    expect(
      wrapper.find("input[name='restTimePerRound']").props().value
    ).to.be.equal(60)
    expect(
      wrapper.find("input[name='restTimePerExercise']").props().value
    ).to.be.equal(20)

    // Shows 'Next »' button
    expect(wrapper.find("button[type='submit']").props().children).to.be.equal(
      'Next »'
    )
    ...
    expect(
      wrapper
        ...
        .last()
        .props().children
    ).to.be.equal('« Previous')
    expect(wrapper.find("button[type='submit']").props().disabled).to.be.false
    expect(wrapper.find("button[type='submit']").text()).to.be.equal(
      props.submitText
    )
    ...
    expect(
      wrapper.find("input[name='exercises.0.quantity']").props().value
    ).to.be.equal(10)
    expect(
      wrapper.find(`select[name='exercises.0.quantityUnit']`).props().value
    ).to.be.equal(UNITS.REPS.value)
    expect(
      wrapper.find("input[name='exercises.0.weight']").props().value
    ).to.be.equal(0)
    expect(
      wrapper.find("input[name='exercises.0.weightUnit']").props().value
    ).to.be.equal(UNITS.KG.value)
    ...
    wrapper
      ...
      .first()
      .simulate('click')
    expect(wrapper.find(AsyncCreatableSelect).text()).to.be.equal(
      exercises[0].name
    )
  ...
  it('can update an existing workout', async () => {
    ...
    wrapper
      .find("input[name='name']")
      .simulate('change', getEvent('name', newWorkout.name))
    wrapper
      .find("input[name='rounds']")
      .simulate('change', getEvent('rounds', newWorkout.rounds))
    wrapper.find('form').simulate('submit')
    await tick()
    wrapper.update()
    ...
    wrapper
      ...
      .simulate('click')
    wrapper
      .find("input[name='exercises.0.quantity']")
      .simulate(
        'change',
        getEvent('exercises.0.quantity', newWorkout.exercises[0].quantity)
      )
    wrapper
      .find("input[name='exercises.0.weight']")
      .simulate(
        'change',
        getEvent('exercises.0.weight', newWorkout.exercises[0].weight)
      )
