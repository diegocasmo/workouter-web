...
describe('<WorkoutExerciseForm/>', () => {
  ...
  it('renders', () => {
    ...
    expect(wrapper.find(AsyncCreateSelect).props().name).to.be.equal(
      `exercises.${props.index}.name`
    )
    expect(wrapper.find(AsyncCreateSelect).props().value).to.be.equal(
      props.exerciseName
    )
    expect(wrapper.find(AsyncCreateSelect).props().defaultOptions).to.be.true

    // Quantity <Input/>
    const Quantity = wrapper.findWhere(
      c => c.props().name === `exercises.${props.index}.quantity`
    )
    ...
    const QuantityUnit = wrapper.findWhere(
      c => c.props().name === `exercises.${props.index}.quantityUnit`
    )
    expect(QuantityUnit.props().label).to.be.equal('Quantity unit')
    expect(QuantityUnit.props().options).to.be.eql(getUnits())

    // Weight <Input/>
    const Weight = wrapper.findWhere(
      c => c.props().name === `exercises.${props.index}.weight`
    )
    ...
    // Weight Unit <Input/>
    const WeightUnit = wrapper.findWhere(
      c => c.props().name === `exercises.${props.index}.weightUnit`
    )
