...
describe('<ExerciseItem/>', () => {
  ...
  it('renders', () => {
    ...
    expect(wrapper.find('.wkr-exercise-item__name').text()).to.be.equal(
      props.exercise.name
    )
    expect(
      wrapper.find({ to: `/exercises/update/${props.exercise.id}` })
    ).to.have.lengthOf(1)
  })
  ...
  describe('handleDeleteExercise', () => {
    ...
    it("calls 'handleDeleteExercise' when action is clicked and confirmed", () => {
      ...
      expect(props.handleDeleteExercise.calledWith(props.exercise.id)).to.be
        .true
    })
