...
describe('Workout', () => {
  ...
  describe('fetchWorkouts()', () => {
    ...
    it('returns paginated workouts', async () => {
      ...
      await db.workouts.bulkAdd(Factory.buildList('workout', 20, {}, {except: ['id']}))
      ...
      const sortedWorkouts = [...allWorkouts].sort((a, b) => a.name.localeCompare(b.name))
    ...
    it('returns paginated workouts filtered by name', async () => {
      ...
      workouts.forEach((_, i) => { workouts[i].name = names[i] })
      await db.workouts.bulkAdd(workouts)

      // Retrieve paginated workouts filtered by their names
      const firstPage = await fetchWorkouts({name: 'a', pageNum: 0, perPage: 3, db})
      const secondPage = await fetchWorkouts({name: 'a', pageNum: 1, perPage: 3, db})
