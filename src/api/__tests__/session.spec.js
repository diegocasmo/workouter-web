...
describe('Session', () => {
  describe('validateSession()', () => {
    ...
    it('returns the session if valid', () => {
      ...
      return validateSession(attrs).then(res => expect(res).to.be.eql(attrs))
    })
  })
  ...
  describe('fetchSessions()', () => {
    it('returns paginated sessions', async () => {
      ...
      await db.sessions.bulkAdd(
        Factory.buildList('session', 20, {}, { except: ['id'] })
      )
      ...
      expect(firstPage).to.be.eql(
        [...allSessions.slice(10)].sort((a, b) => b.id - a.id)
      )
      expect(secondPage).to.be.eql(
        [...allSessions.slice(0, 10)].sort((a, b) => b.id - a.id)
      )
    })
  })
