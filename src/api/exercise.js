...
export function validateExercise(attrs) {
  return ExerciseSchema.validate(attrs).catch(yupError =>
...
export async function getExercise(id, db = connection) {
  ...
  return exercise
    ? exercise
    : Promise.reject(new Error(`Exercise ${id} doesn't exist`))
}
...
export async function deleteExercise(id, db = connection) {
  ...
  return deleted
    ? deleted
    : Promise.reject(new Error(`Unable to delete exercise ${id}`))
}
