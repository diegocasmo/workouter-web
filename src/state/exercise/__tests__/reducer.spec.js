...
describe('Exercise Reducer', () => {
  it('should return the initial state', () => {
    expect(exerciseReducer(undefined, {})).to.be.eql({
      perPage: 12,
      hasMore: true,
      items: {},
  ...
  describe('FETCH', () => {
    it('FETCH_INIT', () => {
      ...
      expect(exerciseReducer(state, action)).to.be.eql({
        ...state,
        items,
    ...
    it('FETCH_SUCCESS', () => {
      ...
      expect(exerciseReducer(initialState, action)).to.be.eql(expectedState)
    })
  ...
  describe('GET', () => {
    it('GET_INIT', () => {
      ...
      expect(exerciseReducer(initialState, action)).to.be.eql({
        ...initialState,
        status: REQUEST_STATUS.GET,
      })
    })
    ...
    it('GET_SUCCESS', () => {
      ...
      const nextExercise = Factory.build('exercise', {
        ...prevExercise,
        name: 'new version',
      })
      const state = {
  ...
  describe('DELETE', () => {
    ...
    it('DELETE_SUCCESS', () => {
      ...
      const prevItems = exercises.reduce(
        (acc, x) => ({ ...acc, [x.id]: x }),
        {}
      )
      ...
      expect(exerciseReducer(state, action)).to.be.eql(expectedState)
    })
